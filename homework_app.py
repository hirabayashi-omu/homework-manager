...
with tabs[1]:
    ...
    with left:
        st.subheader("宿題の登録")
        for key, default in [
            ("input_subject", ""), ("input_new_subject",""),
            ("input_content",""), ("input_due", date.today()),
            ("input_status","未着手"), ("input_submit_method","Teams"),
            ("input_submit_method_detail","")
        ]:
            if key not in st.session_state:
